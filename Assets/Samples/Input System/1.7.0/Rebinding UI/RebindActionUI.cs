...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        private void ResetBinding(InputAction action, int bindingIndex)
        {
            ...
            action.RemoveBindingOverride(bindingIndex);
            foreach(InputAction otheAction in action.actionMap.actions)
            {
                if (otheAction == action) continue;

                for(int i = 0; i < otheAction.bindings.Count; i++)
                {
                    InputBinding binding = otheAction.bindings[i];
                    if(binding.overridePath == newBinding.path)
                    {
                        otheAction.ApplyBindingOverride(i, oldOverridePath);
                    }
                }
            }
        }
